...
static fpMat33_t rMat;

#if defined(UNIT_TEST)
void imuSetRotMatrix(const fpMat33_t* inMatrix) { rMat = *inMatrix; }
#endif
...
void gyroSendTo_imu(const fpVector3_t* g, const uint32_t stampCycles, const bool overflow)
{
    IMU_LOCK;
...
{
    vectorZero(&imuGyroRecieveState.gyroData);
...
static void imuReceiveFrom_gyro(fpVector3_t* g, float* deltaTime, float* saturated)
{
    IMU_LOCK;
    vectorScale(g, &imuGyroRecieveState.gyroData, (float)(DEGREES_TO_RADIANS(1.0f)));
...
STATIC_UNIT_TESTED void imuComputeRotationMatrix(fpMat33_t* outMat, const quaternion* q)
{
...
STATIC_UNIT_TESTED void imuCalcAccError(imuAhrsState_t* state, const imuRuntimeConfig_t* config, const float dt, const fpVector3_t* gyro, fpVector3_t acc_bf, const fpMat33_t* rotMat)
{
    UNUSED(dt);

    const float gyroNorm = vectorNorm(gyro);

    // Use measured acceleration vector
    const float accNorm = vectorNorm(&acc_bf);
    vectorNormalize(&acc_bf, &acc_bf);

    // estimated roll pitch vector in body frame
    const fpVector3_t estRP_bf = {.x = rotMat->m[2][0], .y = rotMat->m[2][1], .z = rotMat->m[2][2]};
    ...
    if (accCovariance > 0.0f) {
        ...
        const float accDiffNormSq = vectorNormSquared(&accDiff);
        if (accDiffNormSq > 0.001f) {  // scale larger errors with the actual magnitude of the angle
            vectorScale(&accDiff, &accDiff, accAngleError / sqrtf(accDiffNormSq));
        }
        vectorAdd(&(state->error.rp), &(state->error.rp), &accDiff);
...
STATIC_UNIT_TESTED void imuAhrsUpdate(imuAhrsState_t* state,
                                ...
                                const fpVector3_t* gyro,
                                const float durationSaturated,
                                const fpMat33_t* rotMat)
{
    // Calculate general spin rate (rad/s)
    const float gyroRate = vectorNorm(gyro);

    // Heading error
    fpVector3_t headingError = { .x = rotMat->m[Z][X], .y = rotMat->m[Z][Y], .z = rotMat->m[Z][Z] };
    vectorScale(&headingError, &headingError, state->error.heading);
    fpVector3_t headingChange;
    vectorScale(&headingChange, &headingError, state->headingGain);
    ...
    fpVector3_t rpChange = state->error.rp;
    vectorScale(&rpChange, &rpChange, rpGain);

    fpVector3_t totalChange;
    vectorAdd(&totalChange, &rpChange, &headingChange);
    ...
    if (config->imuDcmKi > 0.0f) {
        ...
        if (gyroRate < DEGREES_TO_RADIANS(SPIN_RATE_LIMIT)) {
            fpVector3_t iChange;
            vectorAdd(&iChange, &state->error.rp, &headingError);
            vectorScale(&iChange, &iChange, config->imuDcmKi * dt);  // integral error scaled by Ki
            vectorAdd(&state->nominal.integralErr, &state->nominal.integralErr, &iChange);
        }
    } else {
        vectorZero(&state->nominal.integralErr);  // prevent integral windup
    }

    // Add errors and integrate rate of change of quaternion
    fpVector3_t g = { .x = 0.0f, .y = 0.0f, .z = 0.0f };
    vectorAdd(&g, gyro, &state->nominal.integralErr);
    vectorScale(&g, &g, dt);
    vectorAdd(&g, &g, &totalChange);

    vectorScale(&g, &g, 0.5f);
...
STATIC_UNIT_TESTED void imuUpdateEulerAngles(const fpMat33_t* rotMat)
{
...
STATIC_UNIT_TESTED void imuCalcCourseErr(imuAhrsState_t* state, const imuRuntimeConfig_t* config, const float dt, float courseOverGround, const fpMat33_t* rotMat)
{
    ...
    fpVector2_t heading_ef = {.x = rotMat->m[X][X], .y = rotMat->m[Y][X]};
...
// return value rotation around earth Z axis, pointing in directipon of smaller error, [rad/s]
STATIC_UNIT_TESTED void imuCalcMagErr(imuAhrsState_t* state, const imuRuntimeConfig_t* config, const float dt, const fpVector3_t* mag_bf, const fpMat33_t* rotMat)
{
...
static void imuCalculateEstimatedAttitude(timeUs_t currentTimeUs)
{
    ...
    if (sensors(SENSOR_MAG)
        ...
        useMag = true;
        fpVector3_t mag_bf = {{mag.magADC[X], mag.magADC[Y], mag.magADC[Z]}};
        imuCalcMagErr(&ahrsState, &imuRuntimeConfig, dt, &mag_bf, &rMat);
    }
#endif
...
void imuUpdateAttitude(timeUs_t currentTimeUs)
{
    if (sensors(SENSOR_ACC) && acc.isAccelUpdatedAtLeastOnce && gyroIsCalibrationComplete()) {
        ...
        if (imuRuntimeConfig.throttleAngleValue && (FLIGHT_MODE(ANGLE_MODE) || FLIGHT_MODE(HORIZON_MODE)) && ARMING_FLAG(ARMED)) {
