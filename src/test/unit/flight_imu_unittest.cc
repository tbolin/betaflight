...
extern "C" {
    ...
    void imuComputeRotationMatrix(fpMat33_t* outMat, const quaternion* q);
    void imuUpdateEulerAngles(const fpMat33_t* rotMat);
    ...
    void imuCalcAccError(imuAhrsState_t* state, const imuRuntimeConfig_t* config, const float dt, const fpVector3_t* gyro, fpVector3_t acc_bf, const fpMat33_t* rotMat);
    void imuCalcMagErr(imuAhrsState_t* state, const imuRuntimeConfig_t* config, const float dt, const fpVector3_t* mag_bf, const fpMat33_t* rotMat);
    void imuCalcCourseErr(imuAhrsState_t* state, const imuRuntimeConfig_t* config, const float dt, float courseOverGround, const fpMat33_t* rotMat);
    
    void imuResetAhrsState(imuAhrsState_t* state);
    void imuSetQuaternion(const quaternion* inQuat);
    void imuSetRotMatrix(const fpMat33_t* inMatrix);
    // extern quaternion q;
